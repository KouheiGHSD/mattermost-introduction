...
## まえがき
社内で完結できるChatOps環境を作りたかった。
Kandan, Let's chatなどを試す中、Mattermostがもっとも触り心地がよかった。
* チャットとしての機能が最初からあり、日本語翻訳が当時からあった
* セットアップもDockerを使ってすぐ構築できた
* 開発がオープンに進められており、コミュニティもアクティブだった
* サーバーサイドはGo言語、フロントエンドはReact.jsなど、自分が興味を持っている技術が使われていて魅力的だった
些細な翻訳ミスの修正から日本語翻訳をはじめ、今でも時間を見つけてコントリビュートを行なっている。
コアチームのチャットではThank youチャンネルを作り、感謝の意を伝え合うなど雰囲気が良い
また、たまに手書きのメッセージカードなども送ってくれる。
他にもコントリビューターギフトとしてマグカップやステッカー、Tシャツなどももらっている。嬉しい。
