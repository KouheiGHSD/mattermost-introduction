...
## まえがき
私とMattermostの出会いは、ChatOpsが流行り始めた頃に「社内でChatOpsを試したいけど社内の情報は外には出せない」という想いからOSSのチャットツールを探している時でした。
Dockerコマンドで簡単に環境構築が可能で、チャットとしての操作性はもちろんのことHubotとの連携も問題なく行うことができ、何よりバックエンドにGo言語、フロントエンドにReact.jsという新しい技術を使って開発を進めているところに興味を持ちました。
その後、ちょっとした翻訳ミスを報告したことから本体へのコントリビュートも行うようになり、日本語翻訳の公式レビュワーやMattermost v4.6リリース時のMVPにも選ばれるなど、利用者としても開発者としても思い入れを深めていきました。
また、最近でも活発に開発が行われ、最近追加されたプラグイン機能は他のチャットツールにはない強みであり、この部分にさらなる可能性を感じています。

本連載では、Mattermostの簡単な使い方の紹介からプラグインの開発手法までを紹介していく予定です。
...
今回はMattermotの基本的な機能や、弊社での利用事例について紹介します。
...
ユニコーン企業として有名なUber社も、SlackやHipChatがベースとなっていたコミュニケーション基盤を、MattermostベースのuChatという独自のコミュニケーション基盤に置き換えるため開発を進めていると公言しています。
